...
from storeapi.database import post_table,comment_table,database,like_table
...
async def find_post(post_id: int):
    ...
    query=post_table.select().where(post_table.c.id==post_id) #sqlalchemy.select(post_table).select_from(post_table)
    logger.debug(query)
...
async def create_comment(comment: CommentIn,current_user:Annotated[User,Depends(get_current_user)]):
    logger.info("creating commments")
